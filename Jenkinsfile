    ...
    stages {
        stage('Build') {
            steps {
                ...
                sh  '''
                    docker run --user $(id -u):$(id -g) --rm --name=ton_console_prod_npm_install -v "$PWD":/local --workdir=/local node:20.17.0-alpine3.20 npm ci
                    docker run --user $(id -u):$(id -g) --rm --name=ton_console_prod_build       -v "$PWD":/local --workdir=/local node:20.17.0-alpine3.20 npm run build
                    '''
