...
2. You must specify the payment recipient's TON wallet address during registration. This address will be used to receive all incoming invoice payments. The recipient wallet must have at least one outgoing transaction on the TON blockchain to be eligible.
If the wallet is new, send a small amount (e.g., 0.01 TON) to any address to activate it.
