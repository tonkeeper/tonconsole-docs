...
1. `host` - address of Claim API: `https://mainnet-airdrop-v2.tonapi.io`
...
function claimJetton(claimId, userAddress) {
    const claimUrl = `https://mainnet-airdrop-v2.tonapi.io/v2/airdrop/claim/${userAddress}?id=${airdropId}`;
    fetch(claimUrl)
