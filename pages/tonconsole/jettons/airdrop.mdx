...
1. `host` - address of Claim API: `https://mainnet-airdrop-v2.tonapi.io` // TODO: remove -v2 fuffix from subdomain later
...
function claimJetton(claimId, userAddress) {
    const claimUrl = `https://mainnet-airdrop-v2.tonapi.io/v2/airdrop/claim/${userAddress}?id=${airdropId}`; // TODO: remove -v2 fuffix from subdomain later
    fetch(claimUrl)
