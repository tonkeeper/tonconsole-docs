...
To configure the claim, use the following URL format:
```
https://tonkeeper.github.io/airdrop-reference-dapp/?claimId=<UUID>&claimJetton=<JettonAddress>
```

- Replace `<UUID>` with the claim's UUID from ton-console.
- Replace `<JettonAddress>` with the address of the Jetton being claimed.
...
#### Exapmle call claim function from dApp
<ExampleTabs>
...
function claimJetton(claimId, userAddress) {
    const claimUrl = `https://mainnet-airdrop.tonapi.io/v1/airdrop/claim/${userAddress}?id=${claimId}`;
    fetch(claimUrl)
...
claimJetton('<UUID of airdrop from tonconsole>', '<User address from tonConnectUI>');
```
  </ExampleTab>
</ExampleTabs>
