...
These Jetton Airdrop T&C Terms shall be read in conjunction with the [Airdrop Terms of Use](./terms.md)

Service enables a mass token distribution model where the recipient (**Recipient**) pays a fixed fee in TONs and receives Jettons.
...
`distributor_balance_change` = +0.112809142 TON and slightly less than the **Excess** due to blockchain fees for the payment of which the **Airdrop admin** is responsible.
The estimate amount could be seen in the `Value flow` tab of the given **Claim** operation example.

The TONs received by the distributor contract are divided in the following proportion:

`royalty_receiver_share = royalty_coefficient * excess`

`admin_share = excess - royalty_receiver_share - blockchain_fee` (**Airdrop admin share**), where the royalty receiver share is the fees for Scalable Data Solutions Inc.

The blockchain fees is charged both at the time of receiving **Excess** and during subsequent transactions made with distributor contracts.
...
#### 423 (Locked)
* `{"code":1, "message":"not processed yet"}` - the **Airdrop** data has not yet been processed. Wait for processing. The error CANNOT appear if the workflow in the TonConsole is followed.
* `{"code":2, "message":"locked"}` - the error occurs if the **Claim** is set to `Disabled` in the TonConsole.
