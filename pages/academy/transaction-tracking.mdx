...
4. You can’t query a message by its hash using a light server – they don’t index that. You’ll need a full blockchain index (like tonapi.io, toncenter.com, or dton.io).
...
<ExampleTabs>
  <ExampleTab label="Go (tongo)">
    ...
    // msg is *tlb.Message constructed manually or parsed from BOC
    hash := msg.Hash(true)
    fmt.Printf("%x\n", hash)
    ```
  </ExampleTab>
  <ExampleTab label="Go (tonutils)">
    ```go
    import (
        "github.com/xssnick/tonutils-go/tlb"
        "github.com/xssnick/tonutils-go/boc"
        "fmt"
    )

    // msg is *tlb.ExternalMessage loaded from BOC or constructed manually
  ...
  <ExampleTab label="Python (tonutils)">
    ```python
    from tonutils.message import message_to_boc_hex

    # message: MessageAny instance (already parsed or constructed)
    boc_hex, normalized_hash_hex = message_to_boc_hex(message, normalize_hash=True)
    print(normalized_hash_hex)
    ```
  </ExampleTab>
</ExampleTabs>
...
A **normalized external‑in message hash** can be resolved **only** by full blockchain indexers  
(such as **tonapi.io**, toncenter, or dton). Lite servers and archive nodes do not understand it.  
The examples below use **tonapi.io**.

#### Key REST Endpoints

| Goal | Endpoint |
|------|----------|
| Raw message (cell + metadata) |  [`getBlockchainMessage`](https://tonapi.io/api-v2#operations-Blockchain-getBlockchainMessage) |
| Transaction that consumed the message | [`getBlockchainTransactionByMessageHash`](https://tonapi.io/api-v2#operations-Blockchain-getBlockchainTransactionByMessageHash) |
| Full trace (transaction tree) |  [`getTrace`](https://tonapi.io/api-v2#operations-getTrace) |
| Root transaction directly (if you already know its hash) |  [`getBlockchainTransaction`](https://tonapi.io/api-v2#operations-Blockchain-getBlockchainTransaction) |
...
| Stage | What the API returns | What it means | Recommended UX |
|-------|----------------------|---------------|----------------|
| **Pending** | `traces/{hash}` exists, `"status": "pending"` | Message is in mempool, not yet in a block | Show a tracking link such as `https://tonviewer.com/transaction/{hash}` so the user can watch progress |
| **Finalized** | `"status"` field disappears and trace now contains execution data | Full transaction chain executed | Inspect trace: if all nodes are `success` → show *Success*; otherwise show a specific failure (e.g. *TTL expired*, *bounce*, *insufficient funds*) |

> **Tip:** cache `{hash, expiresAt}` locally. If the trace is still `pending` after `expiresAt` (TTL), mark it as failed automatically.
...
3. **When “pending” appears** — present the Tonviewer link or custom “Pending” badge.  
4. **When finalized** — parse the trace:  
   - All transactions `success` → show confirmation & optional explorer link.  
