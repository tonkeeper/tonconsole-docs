...
| **Finalized** | `"status"` field disappears and trace now contains execution data | Full transaction chain executed           | Inspect trace: if all nodes are `success` → show _Success_; otherwise show a specific failure (e.g. _TTL expired_, _bounce_, _insufficient funds_) |
